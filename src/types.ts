...
/* ── events (the semantic vocabulary) ───────────────────────────────── */
...
export interface BackendProbe {
  kind: BackendKind;
  vibrate?: { exists: boolean; lastReturn: boolean | null };
  iosSwitch?: { available: boolean };
}
...
export type MotionSpec =
  | { kind: "none" }
  | { kind: "boop"; scale?: number; rotate?: number; x?: number; y?: number; timing?: number }
  | {
      kind: "particles";
      emojis?: string[];
  ...
  | { kind: "custom"; play: (target: MotionTarget, runtime: MotionRuntime) => void };
...
/* ── per-event recipe spanning all channels ─────────────────────────── */
...
export type EventOverrides = Partial<{ [K in EventName]: Partial<EventRecipe> }>;
...
export interface TactileReport {
  backend: BackendKind;
  channels: { haptics: boolean; sound: string | false; motion: string | false };
  platform: { ios: boolean; android: boolean; desktop: boolean; standalonePWA: boolean };
  vibrate: { exists: boolean; lastReturn: boolean | null };
  iosSwitch: { available: boolean };
  audio: { enabled: boolean; state: AudioContextState | "absent" };
  reducedMotion: boolean;
  notes: string[];
}
...
export interface Tactile {
  ...
  // Back-compat aliases — drop-in for an existing `haptics.*` facade.
